...
class Submenu_new():
    ...
    def running(self):
        ...
        while running:
            ...
            for i in range(len(self.content)):
                menu_slots[i] = Button(screen, self.content[i].name, BLACK, 18, self.menu_list_locations[i][0], self.menu_list_locations[i][1], self.menu_list_values[i])

            # DRAW
            for i in range(len(menu_slots)):
                menu_slots[i].draw_left()
...
class Battletext():
    ...
    def magical_damage(self, name, damage_done, ability):
        self.text = str(name) + " attacks for " + str(damage_done) + " " + str(ability.name) + " damage!"
        return self.text
...
def game():
    ...
    while running:
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_RETURN:
                        ...
                        if user.menu_location == [1, 1]:
                            damage_done = player.attack_physical()
        ...
        if battle_phase == 2:
            ...
            # EVENTS
            damage_done = target.attack_physical()
